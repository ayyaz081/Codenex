// Authorization check - run immediately
        function checkUserAccess() {
            ...
            // If not admin or manager, redirect to auth page
        ...
        // Don't run authorization check immediately - let it run after DOM loads
        
        // Update UI based on user role
        function updateUIForRole() {
            const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
            const roleBadge = document.getElementById('roleBadge');
            const usersMenuItem = document.querySelector('.nav-link[data-section="users"]')?.parentElement;
            const adminUserDisplay = document.getElementById('adminUserDisplay');
            
            // Update role badge and name display
            if (roleBadge) {
                roleBadge.textContent = userInfo.role === 'Admin' ? 'Administrator' : userInfo.role || 'User';
            }
            if (adminUserDisplay) {
                adminUserDisplay.textContent = userInfo.firstName || 'User';
            }
            
            // Hide user management for non-admin users
            if (userInfo.role !== 'Admin' && usersMenuItem) {
        ...
        let sidebar, mainContent, mobileToggle, sidebarToggle, sidebarClose, mobileOverlay, themeToggle, pageTitle;
        ...
        class AdminAPI {
            static async fetchProducts(domain = '', search = '') {
                try {
                    ...
                    if (search) {
                        ...
                        // Use window.enhancedSearch to ensure it's available
                        if (typeof window.enhancedSearch === 'function') {
                            products = window.enhancedSearch(products, search, 'domain');
                        } else {
                            // Fallback search if enhancedSearch is not available
                            const searchLower = search.toLowerCase();
                            products = products.filter(p => 
                                (p.title && p.title.toLowerCase().includes(searchLower)) ||
                                (p.shortDescription && p.shortDescription.toLowerCase().includes(searchLower)) ||
                                (p.longDescription && p.longDescription.toLowerCase().includes(searchLower)) ||
            ...
            static async fetchSolutions(area = '', search = '') {
                try {
                    ...
                    if (search) {
                        const beforeSearch = solutions.length;
                        if (typeof window.enhancedSearch === 'function') {
                            solutions = window.enhancedSearch(solutions, search, 'problemArea');
                        } else {
                            // Fallback search if enhancedSearch is not available
                            const searchLower = search.toLowerCase();
            ...
            static async fetchPublications(domain = '', search = '') {
                try {
                    ...
                    if (domain && search) {
                        ...
                        // Then filter by search term using enhanced search
                        publications = enhancedSearch(publications, search, 'domain');
                    } else if (domain) {
                        // Only domain filter
            ...
            static async fetchRepository(category = '', type = '') {
                try {
                    ...
                    // Filter by category client-side for better flexibility
                    if (category) {
            ...
            static async fetchContacts(status = '', search = '') {
                try {
                    ...
                    // Client-side search filtering with enhanced search
                    if (search) {
                        contacts = enhancedSearch(contacts, search, 'subject');
                    }
        ...
        function initializeApp() {
            ...
            if (isLoggedIn && currentUser && (currentUser.role === 'Admin' || currentUser.role === 'Manager')) {
                // User is already logged in as admin or manager, show admin interface
                document.querySelector('.admin-container').style.display = 'flex';
                updateAdminDisplay();
            } else {
                // User not logged in or not admin/manager, redirect to auth page
                window.location.href = '/Auth?returnUrl=' + encodeURIComponent(window.location.href);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // For testing purposes, bypass auth temporarily
            // Set up a basic admin user if none exists
            ...
            mobileToggle = document.getElementById('mobileToggle');  // Fixed ID mismatch
            ...
            console.log('Navigation setup - Found nav links:', navLinks.length, 'content sections:', contentSections.length);
            
            // Ensure elements exist
            if (navLinks.length === 0) {
                console.error('No navigation links found - DOM may not be ready');
                return;
            }
            ...
            // Force show admin container for debugging
            ...
            // Initialize sidebar state after setup
            ...
            console.log('Scheduling auto-complete setup...');
            setTimeout(() => {
                console.log('Running auto-complete setup now...');
                setupAutoComplete();
                
                // Run test function after setup
                setTimeout(() => {
                    testAutoComplete();
                }, 2000);
            }, 1000);
            
            // Setup GitHub auto-fetch functionality
            setTimeout(() => {
                console.log('Initializing GitHub auto-fetch...');
                setupGitHubAutoFetch();
            }, 1500);
            
            // Setup shared component integration
            setupSharedComponentBridge();
        });
        
        // Shared component integration
        function setupSharedComponentBridge() {
            // Wire logout button from shared header
            const checkLogoutButton = setInterval(() => {
                const logoutBtn = document.getElementById('logout-btn');
                if (logoutBtn && !logoutBtn.hasAttribute('data-admin-wired')) {
                    logoutBtn.setAttribute('data-admin-wired', 'true');
            ...
            setTimeout(() => clearInterval(checkLogoutButton), 10000);
            setTimeout(updateHeaderAuth, 200);
        }
        ...
        function updateHeaderAuth() {
            ...
            // Update user avatar and info in shared header if elements exist
            ...
            if (isLoggedIn && currentUser) {
                ...
                if (userLink) {
                    ...
                    userLink.title = fullName || 'Admin Account';
                }
            } else {
        ...
        function setupEventListeners() {
            ...
            navLinks.forEach((link, index) => {
                link.addEventListener('click', function(e) {
                    ...
                    // Activate clicked nav and corresponding section
                    ...
                    if (targetSection) {
                        ...
                        // Load section data if function exists
        ...
        function loadSectionData(sectionId) {
            // Define load functions for each section
            switch (sectionId) {
                case 'products':
                    if (typeof loadProducts === 'function') loadProducts();
                    break;
                case 'solutions':
                    if (typeof loadSolutions === 'function') loadSolutions();
                    break;
                case 'publications':
                    if (typeof loadPublications === 'function') loadPublications();
                    break;
                case 'repository':
                    if (typeof loadRepository === 'function') loadRepository();
                    break;
                case 'contacts':
                ...
                case 'aboutpage':
                    ...
                    if (typeof loadTeamManagement === 'function') loadTeamManagement();
                    if (typeof loadTestimonialsManagement === 'function') loadTestimonialsManagement();
                    break;
                default:
                    console.log('No load function defined for section:', sectionId);
            }
        }
        ...
        function setupSidebar() {
            ...
            const mobileToggleBtn = document.getElementById('mobileToggle');  // Fixed ID mismatch
            if (mobileToggleBtn) {
            ...
            // Handle sidebar toggle (collapse/expand on desktop)
        ...
        function setPageTitle(sectionId) {
            ...
            const breadcrumbIcon = document.querySelector('.page-breadcrumb i');
            ...
            if (breadcrumbIcon) {
                breadcrumbIcon.className = sectionIcons[sectionId] || 'fas fa-chart-line';
            }
        }
        ...
        window.addEventListener('resize', function() {
            ...
            if (window.innerWidth > 1024) {
                // Desktop view - remove mobile visibility and restore collapse state
                if (sidebar) {
                    sidebar.classList.remove('visible');
                    // Close mobile overlay
                    const adminOverlay = document.getElementById('mobile-menu-overlay');
                ...
                // Let CSS handle margin changes
                if (mainContent) {
                    mainContent.style.marginLeft = '';
                }
            } else {
                // Mobile view - remove collapsed state and use mobile toggle logic
                if (sidebar) {
                    sidebar.classList.remove('collapsed');
                    sidebar.classList.remove('visible');
                }
        ...
        function closeModal(modalId) {
            ...
            if (modal) {
                ...
                // Allow CSS transitions to handle the closing animation
                
                // Optional: Set display: none after transition completes
                setTimeout(() => {
        ...
        function forceScrollRestore() {
            ...
            // Gently restore scroll without aggressive testing
            document.body.style.overflow = '';
        ...
        // Simple button safety system (non-aggressive)
        document.addEventListener('click', function(e) {
        ...
        function reactivateAllButtons() {
            ...
            // Only remove event blocks if they exist
        ...
        function ensureBodyScrolling() {
            ...
            if (!openModal && !sidebarOpen) {
                ...
                // Clear html element overflow too
                document.documentElement.style.overflow = '';
        ...
        // NUCLEAR: Run cleanup periodically (less aggressive)
        setInterval(() => {
            // Check if there are any open modals
            const openModal = document.querySelector('.modal-overlay.show');
            if (!openModal) {
                // Only run if there seems to be a scroll issue
        ...
        // NUCLEAR: Emergency recovery only for modal-related clicks
        let lastEmergencyFix = 0;
        document.addEventListener('click', function(e) {
            // Only trigger for modal-related elements
            const isModalRelated = e.target.closest('.btn-view') || 
            ...
            if (isModalRelated) {
                const now = Date.now();
                // Only run every 3 seconds to avoid spam
                if (now - lastEmergencyFix > 3000) {
                    lastEmergencyFix = now;
                    setTimeout(() => {
                        const hasOpenModal = document.querySelector('.modal-overlay.show');
                        if (!hasOpenModal) {
                            forceScrollRestore();
        ...
        // Also run when page becomes visible (user switches back to tab)
        document.addEventListener('visibilitychange', function() {
            if (!document.hidden) {
                setTimeout(ensureBodyScrolling, 100);
            }
        });
        
        // Initialize form submission handlers for all modals
        ...
        window.fixAdminScrolling = function() {
            ...
            // Restore body scrolling - comprehensive approach
            document.body.style.overflow = '';
            document.body.style.overflowY = '';
            document.body.style.overflowX = '';
            document.body.style.position = '';
            ...
            document.body.classList.remove('mobile-menu-open', 'modal-open', 'no-scroll');
            
            // Also check main content and html elements
            document.documentElement.style.overflow = '';
            const mainContent = document.getElementById('mainContent');
            if (mainContent) {
        ...
        window.forceScrollRestore = forceScrollRestore;
        ...
        function showAddRepositoryModal() {
            ...
            // Hide GitHub fetch status
            const statusDiv = document.getElementById('githubFetchStatus');
            if (statusDiv) {
                statusDiv.style.display = 'none';
            }
            
            // Reset and setup GitHub auto-fetch listener
            const githubUrlInput = document.getElementById('repositoryGithubUrl');
            if (githubUrlInput) {
                // Reset initialization flag to allow re-initialization
                githubUrlInput.dataset.autoFetchInitialized = 'false';
            }
            setupGitHubAutoFetch();
        ...
        async function submitRepositoryForm(formData) {
            try {
                ...
                console.log(`${isEditing ? 'Updating' : 'Creating'} repository with data:`);
                ...
                // Prepare the data object with correct field names (Pascal case to match DTO)
        ...
        async function populateSolutionProblemAreas(selectedValue) {
            try {
                ...
                const input = document.getElementById('solutionProblemArea');
                const datalist = document.getElementById('solutionProblemAreaList');
                ...
                datalist.innerHTML = '';
                problemAreas.forEach(pa => {
                    const opt = document.createElement('option');
                    opt.value = pa;
                    datalist.appendChild(opt);
                });
            } catch (e) {
                console.warn('Failed to load problem areas, using existing values');
                // Set the value anyway if provided
                if (selectedValue) {
                    const input = document.getElementById('solutionProblemArea');
        ...
        async function editRepositoryItem(id) {
            try {
                ...
                console.log('Repository data loaded for editing:', repository);
                console.log('GitHub URL from server:', repository.gitHubUrl);
                
                // Populate the form with existing data (using camelCase for JSON response fields)
                ...
                document.getElementById('repositoryGithubUrl').value = repository.gitHubUrl || ''; // Fixed: was repository.githubUrl
                
                // Set pricing radio buttons (using camelCase for JSON response fields)
                const freeRadio = document.querySelector('input[name="pricing"][value="free"]');
                ...
                // Reset and setup GitHub auto-fetch listener (in case URL is changed during edit)
                const githubUrlInput = document.getElementById('repositoryGithubUrl');
                if (githubUrlInput) {
                    githubUrlInput.dataset.autoFetchInitialized = 'false';
                }
...
${contact.message}
        ...
        // Populate product domain datalist with API data
        function populateProductDomainSelect(domains) {
            const productDomainList = document.getElementById('productDomainList');
            if (productDomainList && domains.length > 0) {
                // Clear existing options
                productDomainList.innerHTML = '';
                
                // Add domains from API
                domains.forEach(domain => {
                    const option = document.createElement('option');
                    option.value = domain;
                    productDomainList.appendChild(option);
                });
        ...
        // Populate solution problem area datalist with API data
        function populateSolutionProblemAreaSelect(problemAreas) {
            const solutionProblemAreaList = document.getElementById('solutionProblemAreaList');
            if (solutionProblemAreaList && problemAreas.length > 0) {
                // Clear existing options
                solutionProblemAreaList.innerHTML = '';
                
                // Add problem areas from API
                problemAreas.forEach(area => {
                    const option = document.createElement('option');
                    option.value = area;
                    solutionProblemAreaList.appendChild(option);
                });
                console.log(`Loaded ${problemAreas.length} solution problem areas for admin forms`);
            }
        }
        
        // Populate publication domain datalist with API data
        function populatePublicationDomainSelect(domains) {
            const publicationDomainList = document.getElementById('publicationDomainList');
            if (publicationDomainList && domains.length > 0) {
                // Clear existing options
                publicationDomainList.innerHTML = '';
                
                // Add domains from API
                domains.forEach(domain => {
                    const option = document.createElement('option');
                    option.value = domain;
                    publicationDomainList.appendChild(option);
                });
                console.log(`Loaded ${domains.length} publication domains for admin forms`);
            }
        }
        
        // Populate repository category datalist with API data
        function populateRepositoryCategorySelect(categories) {
            const repositoryCategoryList = document.getElementById('repositoryCategoryList');
            if (repositoryCategoryList && categories.length > 0) {
                // Clear existing options
                repositoryCategoryList.innerHTML = '';
                
                // Add categories from API
                categories.forEach(category => {
                    const option = document.createElement('option');
                    option.value = category;
                    repositoryCategoryList.appendChild(option);
                });
        ...
        // Enhanced search functionality that supports dynamic categories
        function enhancedSearch(items, searchTerm, categoryField) {
            ...
            const filteredItems = items.filter(item => {
                ...
                const searchFields = [
                    item.title,
                    item.description || item.shortDescription || item.longDescription || item.summary || item.abstract,
                    item[categoryField] // Dynamic category field (domain, problemArea, category, etc.)
                ];
                
                // Add specific fields based on item type
                if (item.authors) searchFields.push(item.authors);
                if (item.keywords) searchFields.push(item.keywords);
                if (item.tags) searchFields.push(item.tags);
                if (item.technicalStack) searchFields.push(item.technicalStack);
                if (item.name) searchFields.push(item.name); // For contacts
                if (item.email) searchFields.push(item.email);
                if (item.subject) searchFields.push(item.subject);
        ...
        // Make enhancedSearch globally available
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (productDomainFilter) {
                productDomainFilter.addEventListener('change', () => {
            ...
            if (solutionAreaFilter) {
                solutionAreaFilter.addEventListener('change', () => {
            ...
            if (solutionSearch) {
                solutionSearch.addEventListener('input', debounce(() => {
            ...
            if (publicationDomainFilter) {
                publicationDomainFilter.addEventListener('change', () => {
            ...
            if (publicationSearch) {
                publicationSearch.addEventListener('input', debounce(() => {
            ...
            if (repositoryCategoryFilter) {
                repositoryCategoryFilter.addEventListener('change', () => {
            ...
            if (repositoryTypeFilter) {
                repositoryTypeFilter.addEventListener('change', () => {
            ...
            if (contactSearch) {
                contactSearch.addEventListener('input', debounce(() => {
            ...
            setTimeout(() => {
                if (window.location.search.includes('test=true') || localStorage.getItem('adminTestMode') === 'true') {
                    testDynamicCategoriesImplementation();
                }
            }, 2000);
        });
        ...
        // Setup auto-complete functionality for dynamic input fields
        function setupAutoComplete() {
            // Add input event listeners to update datalists dynamically
            const inputs = [
                { id: 'productDomain', datalistId: 'productDomainList' },
                { id: 'solutionProblemArea', datalistId: 'solutionProblemAreaList' },
                { id: 'publicationDomain', datalistId: 'publicationDomainList' },
                { id: 'repositoryCategory', datalistId: 'repositoryCategoryList' }
            ];
            ...
            inputs.forEach(config => {
                ...
                if (input && datalist) {
                    input.addEventListener('input', function(e) {
                        ...
                        if (value.length >= 2) {
                            // Add the custom value to the datalist if it doesn't exist
                            const existingOption = Array.from(datalist.options)
                                .find(option => option.value.toLowerCase() === value.toLowerCase());
                            
                            if (!existingOption && value.length <= 100) {
                                const newOption = document.createElement('option');
                                newOption.value = value;
                                datalist.appendChild(newOption);
                                
                                console.log(`Added new ${config.id} option: ${value}`);
                            }
                        }
                    });
                    ...
                    input.addEventListener('focus', function(e) {
                        if (e.target.value.trim() === '') {
                            // Show placeholder text or popular suggestions
                            console.log(`Focus on ${config.id} - showing suggestions`);
                        }
                    });
                }
            });
        ...
        // Enhanced search suggestions for main search bars
        function setupSearchSuggestions() {
            const searchInputs = [
                { id: 'productSearch', type: 'products' },
                { id: 'solutionSearch', type: 'solutions' },
                { id: 'publicationSearch', type: 'publications' },
            ...
            searchInputs.forEach(config => {
                ...
                if (searchInput) {
                    // Add placeholder hints based on type
                    const placeholders = {
                        'products': 'Search by title, domain, or description...',
                        'solutions': 'Search by title, problem area, or summary...',
                        'publications': 'Search by title, authors, domain, or keywords...',
        ...
        // Form Event Handlers - REMOVED TO PREVENT DUPLICATES
        // All form event listeners are now handled in setupFormEventListeners() only
        ...
        // Test and verification functions for dynamic categories implementation
        function testDynamicCategoriesImplementation() {
            console.log('=== Testing Dynamic Categories Implementation ===');
            
            // Test 1: Check if input fields replaced select dropdowns
            const testFields = [
                { id: 'productDomain', datalistId: 'productDomainList', type: 'Product Domain' },
                { id: 'solutionProblemArea', datalistId: 'solutionProblemAreaList', type: 'Solution Problem Area' },
                { id: 'publicationDomain', datalistId: 'publicationDomainList', type: 'Publication Domain' },
                { id: 'repositoryCategory', datalistId: 'repositoryCategoryList', type: 'Repository Category' }
            ];
            ...
            // Test 5: Verify that filters are still dynamic
            const filterFields = ['productDomainFilter', 'solutionAreaFilter', 'publicationDomainFilter', 'repositoryCategoryFilter'];
        ...
        // Function to simulate adding a new custom category
        function testAddCustomCategory(inputId, customValue) {
            const input = document.getElementById(inputId);
            if (input) {
                console.log(`Testing custom category addition for ${inputId}:`, customValue);
                ...
                setTimeout(() => {
                    ...
                    if (datalist) {
                        const hasNewOption = Array.from(datalist.options).some(option => 
        ...
        window.testSearch = function(searchType, searchValue) {
            ...
            const inputId = searchInputs[searchType.toLowerCase()];
            if (!inputId) {
                console.error('Invalid search type. Use: products, solutions, publications, or contacts');
                return;
            }
        ...
        // Test function to verify all search inputs exist
        window.checkSearchInputs = function() {
            const inputs = {
                'productSearch': 'Products',
                'solutionSearch': 'Solutions',
                'publicationSearch': 'Publications',
                'contactSearch': 'Contacts'
            };
            
            console.log('=== Search Input Status ===');
        ...
        window.testDynamicCategoriesImplementation = testDynamicCategoriesImplementation;
        window.testAddCustomCategory = testAddCustomCategory;
        
        // Test repository form field mapping
        window.testRepositoryFormMapping = function() {
            console.log('=== Testing Repository Form Field Mapping ===');
            ...
            console.log('Expected JSON response format:', testData);
            
            // Test form field reading
            console.log('\n--- Testing Form Field Reading ---');
            const form = document.getElementById('repositoryForm');
            if (form) {
                const formData = new FormData(form);
                console.log('Form fields can be read:', {
                    title: !!document.getElementById('repositoryTitle'),
                    description: !!document.getElementById('repositoryDescription'),
                    tags: !!document.getElementById('repositoryTags'),
                    githubUrl: !!document.getElementById('repositoryGithubUrl'),
                    license: !!document.getElementById('repositoryLicense'),
                    version: !!document.getElementById('repositoryVersion'),
                    category: !!document.getElementById('repositoryCategory'),
                    technicalStack: !!document.getElementById('repositoryTechnicalStack')
                });
            }
            
            // Test edit function field mapping
            console.log('\n--- Testing Edit Function Field Mapping ---');
            console.log('Edit function will read from JSON response:');
            console.log('  repository.gitHubUrl (camelCase with capital H)');
            console.log('  repository.technicalStack (camelCase)');
            console.log('  repository.isFree (camelCase)');
            console.log('  repository.isPremium (camelCase)');
            
            console.log('\n--- Field Name Mapping Summary ---');
            console.log('HTML form name -> FormData key -> JSON payload key:');
            console.log('  githubUrl -> githubUrl -> GitHubUrl');
            console.log('  technicalStack -> technicalStack -> TechnicalStack');
            console.log('  pricing -> pricing -> IsFree/IsPremium');
            
            console.log('\nJSON response key -> Edit function reads:');
            console.log('  gitHubUrl (from server) -> repository.gitHubUrl');
            console.log('  technicalStack (from server) -> repository.technicalStack');
            console.log('  isFree (from server) -> repository.isFree');
            
            return 'Repository form mapping test completed - check console for details';
        };
        
        // Test all the fixes applied
        window.testAdminFixes = function() {
            console.log('=== Testing Admin Panel Fixes ===');
            
            // Test 1: Check for duplicate event listeners
            console.log('\n--- Testing Duplicate Form Listeners ---');
            const repositoryForm = document.getElementById('repositoryForm');
            if (repositoryForm) {
                const listeners = repositoryForm.cloneNode().getEventListeners?.() || 'Event listeners check not available in this browser';
                console.log('Repository form listeners:', listeners);
            }
            
            // Test 2: Check modal functions
            console.log('\n--- Testing Modal Functions ---');
            console.log('showModal function type:', typeof showModal);
            console.log('closeModal function type:', typeof closeModal);
            console.log('forceScrollRestore function type:', typeof forceScrollRestore);
            
            // Test 3: Check for any open modals
            console.log('\n--- Testing Modal State ---');
            const openModals = document.querySelectorAll('.modal-overlay.show');
            console.log('Currently open modals:', openModals.length, Array.from(openModals).map(m => m.id));
            
            // Test 4: Check body scroll state
            console.log('\n--- Testing Scroll State ---');
            ...
            const editButtons = document.querySelectorAll('.btn-warning[onclick*="edit"]');
            console.log('Edit buttons found:', editButtons.length);
            editButtons.forEach((btn, i) => {
                console.log(`Edit button ${i + 1}:`, btn.getAttribute('onclick'), 'disabled:', btn.disabled);
            });
            
            console.log('\n=== All Tests Complete ===');
            return {
                repositoryForm: !!repositoryForm,
                modalFunctions: typeof showModal === 'function',
                openModals: openModals.length,
                scrollState: document.body.style.overflow || 'default',
                editButtons: editButtons.length
            };
        };
        
        // Test modal state specifically
        window.testModalState = function() {
            ...
            console.log('\n--- View Modal Status ---');
            if (viewModal) {
                console.log('View modal exists:', true);
                console.log('Has show class:', viewModal.classList.contains('show'));
                console.log('Display style:', viewModal.style.display || 'default');
                console.log('Computed display:', window.getComputedStyle(viewModal).display);
                console.log('Classes:', Array.from(viewModal.classList));
            } else {
                console.log('View modal exists:', false);
            }
            ...
            allModals.forEach((modal, i) => {
                console.log(`Modal ${i + 1} (${modal.id || 'no-id'}):`, {
                    hasShow: modal.classList.contains('show'),
                    display: modal.style.display || 'default',
                    computed: window.getComputedStyle(modal).display
                });
            });
            
            return {
                viewModalExists: !!viewModal,
                totalModals: allModals.length,
                openModals: document.querySelectorAll('.modal-overlay.show').length
            };
        };
        ...
        // Initialize admin-specific header customizations after shared components load
        function initializeAdminHeader() {
            // Show logged in state in header
            updateHeaderAuthState();
            
            // Override the shared components logout handler with admin-specific one
            const headerLogoutBtn = document.getElementById('logout-btn');
            if (headerLogoutBtn) {
                // Remove any existing listeners and add admin logout
                headerLogoutBtn.onclick = null;
                headerLogoutBtn.removeEventListener('click', window.sharedComponents?.handleLogout);
                headerLogoutBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    logout(); // Use the admin's logout function
                });
            }
            
            // Let shared components handle theme toggle - no override needed
            // Let shared components handle mobile navigation - no override needed
        }
        ...
        function loadTheme() {
            ...
            // Update theme toggle icon when components load
            setTimeout(() => {
        ...
        function updateHeaderAuthState() {
            ...
            if (loggedOutSection && loggedInSection) {
                // Hide login button, show logged in state
                ...
                if (userAvatar) {
                    ...
                    userAvatar.textContent = adminUsername.charAt(0).toUpperCase();
                }
                
                if (userLink) {
                    userLink.title = adminEmail ? `${adminUsername} (${adminEmail})` : adminUsername;
                }
            }
        }
        
        // Auto-complete functionality for dynamic category/domain inputs
        async function setupAutoComplete() {
            try {
                console.log('Setting up auto-complete functionality...');
                
                // Try to get data from Navigation API first
                let categoryData = null;
                try {
                    const response = await fetch(`${API_BASE_URL}/Navigation/all`);
                    if (response.ok) {
                        categoryData = await response.json();
                        console.log('Category data loaded from API:', categoryData);
                    }
                } catch (error) {
                    console.warn('Navigation API not available:', error);
                }
                
                // If Navigation API fails, extract existing values from actual data
                if (!categoryData) {
                    console.log('Falling back to extracting categories from existing data...');
                    categoryData = await extractCategoriesFromData();
                }
                
                // Populate datalists
                populateDatalist('productDomainList', categoryData.ProductDomains || []);
                populateDatalist('solutionProblemAreaList', categoryData.SolutionProblemAreas || []);
                populateDatalist('publicationDomainList', categoryData.PublicationDomains || []);
                populateDatalist('repositoryCategoryList', categoryData.RepositoryCategories || []);
                
                // Add smart input feedback
                setupSmartInputFeedback();
                
                console.log('Auto-complete setup completed successfully');
                
            } catch (error) {
                console.warn('Auto-complete setup failed:', error);
                // Still set up smart input feedback even if data loading fails
                setupSmartInputFeedback();
            }
        }
        
        // Extract categories from existing data as fallback
        async function extractCategoriesFromData() {
            console.log('Extracting categories from existing data...');
            
            const categoryData = {
                ProductDomains: [],
                SolutionProblemAreas: [],
                PublicationDomains: [],
            ...
            try {
                ...
                if (typeof AdminAPI === 'undefined') {
                    console.warn('AdminAPI not available yet, using default categories');
                    throw new Error('AdminAPI not ready');
                }
                
                console.log('Fetching products for domain extraction...');
                const products = await AdminAPI.fetchProducts();
                console.log('Fetched products:', products.length, products);
                
                const productDomains = [...new Set(products
                    .filter(p => p.domain)
                    .map(p => p.domain.trim())
                    .filter(d => d.length > 0)
                )];
                categoryData.ProductDomains = productDomains.sort();
                console.log('Product domains extracted:', productDomains);
                
                console.log('Fetching solutions for problem area extraction...');
                const solutions = await AdminAPI.fetchSolutions();
                console.log('Fetched solutions:', solutions.length, solutions);
                
                const problemAreas = [...new Set(solutions
                    .filter(s => s.problemArea)
                    .map(s => s.problemArea.trim())
                    .filter(p => p.length > 0)
                )];
                categoryData.SolutionProblemAreas = problemAreas.sort();
                console.log('Problem areas extracted:', problemAreas);
                
                console.log('Fetching publications for domain extraction...');
                const publications = await AdminAPI.fetchPublications();
                console.log('Fetched publications:', publications.length, publications);
                
                const publicationDomains = [...new Set(publications
                    .filter(p => p.domain)
                    .map(p => p.domain.trim())
                    .filter(d => d.length > 0)
                )];
                categoryData.PublicationDomains = publicationDomains.sort();
                console.log('Publication domains extracted:', publicationDomains);
                
                console.log('Fetching repositories for category extraction...');
                const repositories = await AdminAPI.fetchRepository();
                console.log('Fetched repositories:', repositories.length, repositories);
                
                const categories = [...new Set(repositories
                    .filter(r => r.category)
                    .map(r => r.category.trim())
                    .filter(c => c.length > 0)
                )];
                categoryData.RepositoryCategories = categories.sort();
                console.log('Repository categories extracted:', categories);
                
                console.log('Final extracted categories:', categoryData);
                
            } catch (error) {
                console.warn('Error extracting categories from data:', error);
                // Provide some default categories if extraction fails
                categoryData.ProductDomains = ['Cloud Migration', 'Cybersecurity', 'Data Analytics', 'AI/ML', 'Web Development'];
                categoryData.SolutionProblemAreas = ['Cloud Migration', 'Security Assessment', 'Data Analysis', 'Performance Optimization', 'Integration'];
                categoryData.PublicationDomains = ['Cloud Computing', 'Cybersecurity', 'Data Science', 'Machine Learning', 'Software Engineering'];
                categoryData.RepositoryCategories = ['Web Applications', 'Mobile Apps', 'Data Tools', 'Security Tools', 'DevOps', 'Libraries'];
                console.log('Using default categories:', categoryData);
            }
            
            return categoryData;
        }
        
        // Refresh auto-complete data (call this after adding new items)
        async function refreshAutoCompleteData() {
            console.log('Refreshing auto-complete data...');
            try {
                const categoryData = await extractCategoriesFromData();
                
                // Update all datalists with fresh data
                populateDatalist('productDomainList', categoryData.ProductDomains || []);
                populateDatalist('solutionProblemAreaList', categoryData.SolutionProblemAreas || []);
                populateDatalist('publicationDomainList', categoryData.PublicationDomains || []);
                populateDatalist('repositoryCategoryList', categoryData.RepositoryCategories || []);
                
                console.log('Auto-complete data refreshed successfully');
            } catch (error) {
                console.warn('Failed to refresh auto-complete data:', error);
            }
        }
        
        // Direct functions to load domains when modals open
        async function loadProductDomains() {
            try {
                console.log('Loading product domains...');
                const products = await AdminAPI.fetchProducts();
                const domains = [...new Set(products
                    .filter(p => p.domain && p.domain.trim())
                    .map(p => p.domain.trim())
                )].sort();
                
                console.log('Found product domains:', domains);
                
                const datalist = document.getElementById('productDomainList');
                if (datalist) {
                    datalist.innerHTML = '';
                    domains.forEach(domain => {
                        const option = document.createElement('option');
                    ...
                    console.log(`Loaded ${domains.length} product domains into datalist`);
                } else {
                    console.error('productDomainList datalist not found');
                }
        ...
        async function loadSolutionProblemAreas() {
            try {
                ...
                const solutions = await AdminAPI.fetchSolutions();
                const areas = [...new Set(solutions
                    .filter(s => s.problemArea && s.problemArea.trim())
                    .map(s => s.problemArea.trim())
                )].sort();
                
                console.log('Found problem areas:', areas);
                ...
                if (datalist) {
                    ...
                    console.log(`Loaded ${areas.length} problem areas into datalist`);
                }
        ...
        async function loadPublicationDomains() {
            try {
                ...
                const publications = await AdminAPI.fetchPublications();
                const domains = [...new Set(publications
                    .filter(p => p.domain && p.domain.trim())
                    .map(p => p.domain.trim())
                )].sort();
                
                console.log('Found publication domains:', domains);
                
                const datalist = document.getElementById('publicationDomainList');
                if (datalist) {
                    datalist.innerHTML = '';
                    domains.forEach(domain => {
                        const option = document.createElement('option');
                    ...
                    console.log(`Loaded ${domains.length} publication domains into datalist`);
                }
        ...
        async function loadRepositoryCategories() {
            try {
                ...
                console.log('Found repository categories:', categories);
                
                const datalist = document.getElementById('repositoryCategoryList');
                if (datalist) {
                    datalist.innerHTML = '';
                    categories.forEach(category => {
                        const option = document.createElement('option');
                    ...
                    console.log(`Loaded ${categories.length} repository categories into datalist`);
                }
        ...
        // Load Solutions list for dropdown (Publication modal)
        async function loadSolutionsForDropdown() {
            try {
                console.log('Loading solutions for dropdown...');
                const token = getAuthToken();
                const response = await fetch(`${API_BASE_URL}/solutions/list`, {
                    headers: token ? { 'Authorization': `Bearer ${token}` } : {}
                });
                ...
                const select = document.getElementById('publicationSolutionId');
                if (select) {
                    // Keep first option ("Select a Solution")
                    select.innerHTML = '<option value="">Select a Solution</option>';
                    
                    solutions.forEach(solution => {
                        const option = document.createElement('option');
                        option.value = solution.id;
                        option.textContent = solution.title;
                        select.appendChild(option);
                    });
                    
                    console.log(`Loaded ${solutions.length} solutions into dropdown`);
                }
                
                return solutions;
            } catch (error) {
                console.error('Error loading solutions for dropdown:', error);
        ...
        // Manual test function for debugging auto-complete
        function testAutoComplete() {
            console.log('=== Testing Auto-Complete Functionality ===');
            
            // Test 1: Check if datalist elements exist
            const datalists = ['productDomainList', 'solutionProblemAreaList', 'publicationDomainList', 'repositoryCategoryList'];
            datalists.forEach(id => {
                const element = document.getElementById(id);
                console.log(`Datalist ${id}:`, element ? 'EXISTS' : 'NOT FOUND');
                if (element) {
            ...
            // Test 2: Check if input elements exist
            const inputs = ['productDomain', 'solutionProblemArea', 'publicationDomain', 'repositoryCategory'];
            inputs.forEach(id => {
                const element = document.getElementById(id);
                console.log(`Input ${id}:`, element ? 'EXISTS' : 'NOT FOUND');
                if (element) {
                    console.log(`  List attribute:`, element.getAttribute('list'));
                }
            });
            
            // Test 3: Add some test data manually
            console.log('Adding test data to datalists...');
            populateDatalist('productDomainList', ['Test Domain 1', 'Test Domain 2', 'Cloud Migration']);
            populateDatalist('solutionProblemAreaList', ['Test Problem 1', 'Security Assessment']);
            
            console.log('=== Auto-Complete Test Complete ===');
        }
        
        // Test function to load domains manually
        async function testLoadDomains() {
            console.log('=== Testing Manual Domain Loading ===');
            
            try {
                console.log('Loading product domains...');
                const productDomains = await loadProductDomains();
                console.log('Product domains loaded:', productDomains);
                
                console.log('Loading solution problem areas...');
                const problemAreas = await loadSolutionProblemAreas();
                console.log('Problem areas loaded:', problemAreas);
                
                console.log('Loading publication domains...');
                const publicationDomains = await loadPublicationDomains();
                console.log('Publication domains loaded:', publicationDomains);
                
                console.log('Loading repository categories...');
                const repoCategories = await loadRepositoryCategories();
                console.log('Repository categories loaded:', repoCategories);
                
            } catch (error) {
                console.error('Error in manual domain loading test:', error);
            }
            
            console.log('=== Manual Domain Loading Test Complete ===');
        }
        
        // Setup smart feedback for new vs existing values
        function setupSmartInputFeedback() {
            const inputs = [
                { id: 'productDomain', datalistId: 'productDomainList' },
                { id: 'solutionProblemArea', datalistId: 'solutionProblemAreaList' },
                { id: 'publicationDomain', datalistId: 'publicationDomainList' },
                    ...
                    input.addEventListener('input', function() {
                        ...
                        if (icon) {
                            if (value === '') {
                                ...
                                icon.title = 'Select from existing or enter new';
                            } else if (isExisting) {
                                icon.className = 'fas fa-check-circle';
                                icon.style.color = 'var(--success)';
                                icon.title = 'Existing value selected';
                            } else {
                                icon.className = 'fas fa-plus-circle';
                                icon.style.color = 'var(--primary)';
                                icon.title = 'New value - will be added';
                            }
                        }
                    });
                }
        ...
        function populateDatalist(datalistId, options) {
            console.log(`Attempting to populate ${datalistId} with options:`, options);
            
            const datalist = document.getElementById(datalistId);
            if (!datalist) {
                console.warn(`Datalist with ID ${datalistId} not found in DOM`);
                console.log('Available elements:', document.querySelectorAll('datalist'));
                return;
            }
            
            console.log(`Found datalist element:`, datalist);
            ...
            if (options && options.length > 0) {
                ...
                console.log(`Successfully populated ${datalistId} with ${options.length} options:`, options);
            } else {
                console.log(`No options provided for ${datalistId}`);
            }
            
            // Verify the options were added
            const addedOptions = Array.from(datalist.options).map(opt => opt.value);
            console.log(`Final options in ${datalistId}:`, addedOptions);
        }
        
        // Enhanced search function for dynamic filtering
        function enhancedSearch(items, searchTerm, categoryField) {
            if (!searchTerm) {
                return items;
            }
            ...
            return items.filter(item => {
                ...
                // Search in description/summary/abstract
                const descriptionFields = ['shortDescription', 'longDescription', 'summary', 'abstract', 'description'];
                ...
                // Search in category/domain field
                if (categoryField && item[categoryField] && item[categoryField].toLowerCase().includes(query)) {
                    return true;
                }
                
                // Search in other relevant fields
                const otherFields = ['tags', 'authors', 'keywords', 'technicalStack', 'contactName', 'contactEmail'];
        ...
        // Update filter functions to work with dynamic categories
        function filterByCategory(items, categoryValue, categoryField) {
            if (!categoryValue || categoryValue === '') {
                return items;
            }
            
            return items.filter(item => {
                return item[categoryField] && item[categoryField].toLowerCase().includes(categoryValue.toLowerCase());
            });
        }
        
        // Setup form event listeners for modal submissions (ENHANCED)
        function setupFormEventListeners() {
            console.log('Setting up form event listeners...');
            
            // Clear existing listeners first to prevent duplicates
            const forms = ['productForm', 'solutionForm', 'publicationForm', 'repositoryForm', 'teamMemberForm', 'testimonialForm', 'userForm', 'carouselForm'];
            ...
            if (repositoryForm) {
                repositoryForm.addEventListener('submit', async (e) => {
                    ...
                    const submitBtn = repositoryForm.querySelector('button[type="submit"]');
        ...
        // Toggle premium fields visibility based on pricing selection
        ...
        async function autoFillRepositoryFromGitHub(githubUrl) {
            const statusDiv = document.getElementById('githubFetchStatus');
            ...
            try {
                ...
                console.log('%c=== Auto-Fill Summary ===', 'color: green; font-weight: bold;');
                console.log('Title:', document.getElementById('repositoryTitle')?.value || '(not filled)');
                ...
                console.error('Error auto-filling from GitHub:', error);
        ...
        function setupGitHubAutoFetch() {
            const githubUrlInput = document.getElementById('repositoryGithubUrl');
            if (!githubUrlInput) {
                console.warn('GitHub URL input not found');
                return;
            }
            
            // Remove existing listeners by checking if already initialized
            if (githubUrlInput.dataset.autoFetchInitialized === 'true') {
                console.log('GitHub auto-fetch already initialized');
                return;
            }
            ...
            const inputHandler = (e) => {
                ...
                console.log('GitHub URL input detected:', url);
                
                // Only trigger if URL looks like a GitHub URL
                if (url && url.includes('github.com/') && url.split('/').length >= 5) {
                    console.log('Valid GitHub URL detected, will fetch in 800ms...');
                    debounceTimer = setTimeout(() => {
                        console.log('Triggering auto-fill for:', url);
                        autoFillRepositoryFromGitHub(url);
                    }, 800); // Wait 800ms after user stops typing
                }
            };
            
            // Paste event for quick paste
            const pasteHandler = (e) => {
                setTimeout(() => {
                    const url = githubUrlInput.value.trim();
                    console.log('Paste detected, URL:', url);
                    
                    if (url && url.includes('github.com/') && url.split('/').length >= 5) {
                        console.log('Valid GitHub URL pasted, fetching immediately...');
                        autoFillRepositoryFromGitHub(url);
                    }
                }, 150); // Wait for paste to complete
            };
            
            // Attach listeners
            githubUrlInput.addEventListener('input', inputHandler);
            githubUrlInput.addEventListener('paste', pasteHandler);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Setup form event listeners first
